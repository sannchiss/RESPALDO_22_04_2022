...
@section('content')
    ...
    <div class="card">
        <div class="card-header bg-danger text-white">
            <h3 class="card-title">Tipo de Gps 
                @if(Permission::hasPermission('create'))
                    <a type="button" class="btn btn-secondary btn-sm float-right show-form" href="{{ route('maintainer.device.gps.type.create') }}">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        table = $('.table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('maintainer.device.gps.type.index') }}",
            language: {
                "url": dtLanguage
            },
            columns: [
