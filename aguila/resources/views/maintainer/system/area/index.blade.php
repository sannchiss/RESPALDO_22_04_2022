...
@section('content')
    ...
    <div class="card">
        <div class="card-header bg-danger text-white">
            <h3 class="card-title">Areas del sistema 
                @if(Permission::hasPermission('create'))
                    <a type="button" class="btn btn-secondary btn-sm float-right show-form" href="{{ route('maintainer.system.area.create') }}">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        table = $('.table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('maintainer.system.area.index') }}",
            language: {
                "url": dtLanguage
            },
            columns: [
