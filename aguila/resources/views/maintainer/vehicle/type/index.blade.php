...
@section('content')
    ...
    <div class="card">
        <div class="card-header bg-danger text-white">
            <h3 class="card-title">Tipos de Vehiculo 
                @if(Permission::hasPermission('create'))
                    <a type="button" class="btn btn-secondary btn-sm float-right show-form" href="{{ route('maintainer.vehicle.type.create') }}">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        table = $('.table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('maintainer.vehicle.type.index') }}",
            language: {
                "url": dtLanguage
            },
            columns: [
