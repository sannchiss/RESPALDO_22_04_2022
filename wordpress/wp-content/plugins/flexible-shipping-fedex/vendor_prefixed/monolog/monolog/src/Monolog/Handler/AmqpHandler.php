...
use FedExVendor\Monolog\Formatter\JsonFormatter;
use FedExVendor\PhpAmqpLib\Message\AMQPMessage;
use FedExVendor\PhpAmqpLib\Channel\AMQPChannel;
use AMQPExchange;
class AmqpHandler extends \FedExVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct($exchange, $exchangeName = 'log', $level = \FedExVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if ($exchange instanceof \AMQPExchange) {
            $exchange->setName($exchangeName);
        } elseif ($exchange instanceof \FedExVendor\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    {
        return new \FedExVendor\PhpAmqpLib\Message\AMQPMessage((string) $data, array('delivery_mode' => 2, 'content_type' => 'application/json'));
    }
    /**
     * {@inheritDoc}
     */
