...
use FedExVendor\PhpConsole\Connector;
use FedExVendor\PhpConsole\Handler;
use FedExVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \FedExVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \FedExVendor\PhpConsole\Connector $connector = null, $level = \FedExVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('FedExVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\FedExVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FedExVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FedExVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FedExVendor\PhpConsole\Helper::isRegistered()) {
            \FedExVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FedExVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \FedExVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \FedExVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \FedExVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
