 ...
 */
class Logger implements \Psr\Log\LoggerInterface, \FedExVendor\Monolog\ResettableInterface
{
    /**
     ...
     * @return $this
     */
    public function pushHandler(\FedExVendor\Monolog\Handler\HandlerInterface $handler)
    {
    ...
    {
        if (!$this->handlers) {
            $this->pushHandler(new \FedExVendor\Monolog\Handler\StreamHandler('php://stderr', static::DEBUG));
        }
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \FedExVendor\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \FedExVendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName($level)
    {
        if (!isset(static::$levels[$level])) {
            throw new \FedExVendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
