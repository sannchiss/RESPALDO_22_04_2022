            ...
            foreach ($this->reply->RateReplyDetails as $reply_detail) {
                if (!empty($reply_detail->RatedShipmentDetails)) {
                    foreach ($reply_detail->RatedShipmentDetails as $rated_shipment_detail) {
                        if ($this->rate_type === \FedExVendor\FedEx\RateService\SimpleType\RateTypeBasisType::_LIST) {
                            // LIST quotes return both ACCOUNT rates (in RatedShipmentDetails[1])
                            // and LIST rates (in RatedShipmentDetails[3])
                            if (\false === \strpos($rated_shipment_detail->ShipmentRateDetail->RateType, 'PAYOR_LIST')) {
                                break;
                            }
                        }
