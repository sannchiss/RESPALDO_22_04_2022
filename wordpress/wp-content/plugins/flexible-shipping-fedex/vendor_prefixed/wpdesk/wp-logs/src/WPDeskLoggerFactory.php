...
use FedExVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use FedExVendor\WPDesk\Logger\WC\WooCommerceCapture;
use FedExVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \FedExVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\FedExVendor\Monolog\Registry::hasLogger($name)) {
            \FedExVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\FedExVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \FedExVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \FedExVendor\Monolog\Logger($name);
        }
        if (\FedExVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \FedExVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\FedExVendor\Monolog\Logger $logger)
    {
        if (!\defined('FedExVendor\\WC_LOG_THRESHOLD')) {
            \define('FedExVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \FedExVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \FedExVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \FedExVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \FedExVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\FedExVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \FedExVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \FedExVendor\Monolog\Registry::hasLogger($name);
    }
}
