...
use FedExVendor\WPDesk\PluginBuilder\Storage\Exception\ClassNotExists;
 ...
 */
class HelperRemover implements \FedExVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    private function remove_helper_updates()
    {
        ...
        \add_action('plugins_loaded', function () {
            \remove_action('plugins_loaded', [$this->get_helper_instance(), 'init_helper_plugins'], \FedExVendor\WPDesk\Helper\Integration\LicenseIntegration::PRIORITY_HELPER_UPDATE);
        });
    }
    /**
     * @return void
    ...
    {
        $storage = new \FedExVendor\WPDesk\PluginBuilder\Storage\StorageFactory();
        try {
            return $storage->create_storage()->get_from_storage('WPDesk_Helper');
        } catch (\FedExVendor\WPDesk\PluginBuilder\Storage\Exception\ClassNotExists $e) {
            return null;
        }
    }
}
