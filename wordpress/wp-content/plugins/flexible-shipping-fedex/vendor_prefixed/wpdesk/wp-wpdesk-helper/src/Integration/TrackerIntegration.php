 ...
 */
class TrackerIntegration implements \FedExVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /** @var SettingsPage */
    ...
    {
        \add_settings_section('wpdesk_helper_tracking', \__('Plugin usage tracking', 'flexible-shipping-fedex'), null, $this->settings_page->get_page_name());
        \add_settings_field('wpdesk_tracker_agree', \__('Allow WP Desk to track plugin usage', 'flexible-shipping-fedex'), function () {
    ...
    private function handle_render_page_settings_track_section()
    {
        ...
        \printf(\__('No sensitive data is tracked, %sread more%s.', 'flexible-shipping-fedex'), '<a target="_blank" href="' . $terms_url . '">', '</a>');
        ?>
        </p>
		<?php 
    }
