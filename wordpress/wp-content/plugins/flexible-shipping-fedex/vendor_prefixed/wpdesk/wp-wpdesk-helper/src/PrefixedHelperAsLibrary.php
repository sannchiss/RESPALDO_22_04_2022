...
namespace FedExVendor\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use FedExVendor\WPDesk\Helper\Debug\LibraryDebug;
use FedExVendor\WPDesk\Helper\Integration\LicenseIntegration;
use FedExVendor\WPDesk\Helper\Integration\LogsIntegration;
use FedExVendor\WPDesk\Helper\Integration\SettingsIntegration;
use FedExVendor\WPDesk\Helper\Integration\TrackerIntegration;
use FedExVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use FedExVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \FedExVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    {
        $helper_info = new \FedExVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \FedExVendor\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \FedExVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \FedExVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \FedExVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \FedExVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        (new \FedExVendor\WPDesk\Helper\UpgradeSoonNotice())->show_info_about_upgrade_if_old_env();
        $this->clean_wpdesk_menu();
        $library_debug_info = new \FedExVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \FedExVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \FedExVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
