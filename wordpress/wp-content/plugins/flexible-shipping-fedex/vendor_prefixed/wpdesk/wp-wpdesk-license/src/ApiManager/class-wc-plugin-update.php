 ...
 *
 */
if (!\class_exists('FedExVendor\\WPDesk_Update_API_Check')) {
    class WPDesk_Update_API_Check
    {
        ...
        public function plugin_information($args)
        {
            ...
            if (\is_wp_error($request) || \wp_remote_retrieve_response_code($request) != 200) {
                if (\class_exists('FedExVendor\\WPDesk_Logger_Factory')) {
                    if (\is_wp_error($request)) {
                        \FedExVendor\WPDesk_Logger_Factory::log_wp_error($request, \debug_backtrace(\DEBUG_BACKTRACE_PROVIDE_OBJECT | \DEBUG_BACKTRACE_IGNORE_ARGS));
                    } else {
                        \FedExVendor\WPDesk_Logger_Factory::log_message_backtrace('Response is invalid. Response: ' . \json_encode($request), \FedExVendor\WPDesk_Logger::ERROR, \debug_backtrace(\DEBUG_BACKTRACE_PROVIDE_OBJECT | \DEBUG_BACKTRACE_IGNORE_ARGS));
                    }
                }
                return \false;
            }
            ...
            if (\is_object($response)) {
                ...
                if (\class_exists('FedExVendor\\WPDesk_Logger_Factory')) {
                    \FedExVendor\WPDesk_Logger_Factory::log_message_backtrace('Response is not an object', \FedExVendor\WPDesk_Logger::DEBUG, \debug_backtrace(\DEBUG_BACKTRACE_PROVIDE_OBJECT | \DEBUG_BACKTRACE_IGNORE_ARGS));
                } else {
