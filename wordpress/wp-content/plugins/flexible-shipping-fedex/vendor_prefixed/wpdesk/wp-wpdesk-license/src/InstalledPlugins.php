...
class InstalledPlugins
{
    ...
    public function get_plugins_activation_info($hook_to_updates = \false)
    {
        ...
        foreach ($wpdesk_helper_plugins as $key => $wpdesk_helper_plugin) {
            ...
            $addressRepository = new \FedExVendor\WPDesk\License\ServerAddressRepository($wpdesk_helper_plugin['product_id']);
            $plugins[$key] = $wpdesk_helper_plugin;
            $plugins[$key][self::KEY_API_MANAGER] = new \FedExVendor\WPDesk_API_Manager_With_Update_Flag($upgrade_url = $addressRepository->get_default_update_url(), $version = $wpdesk_helper_plugin['version'], $name = $wpdesk_helper_plugin['plugin'], $product_id = $wpdesk_helper_plugin['product_id'], $menu_title, $title = $menu_title, $plugin_file = \basename($wpdesk_helper_plugin['plugin']), $plugin_dir = \dirname($wpdesk_helper_plugin['plugin']), $config_uri, $hook_to_updates);
