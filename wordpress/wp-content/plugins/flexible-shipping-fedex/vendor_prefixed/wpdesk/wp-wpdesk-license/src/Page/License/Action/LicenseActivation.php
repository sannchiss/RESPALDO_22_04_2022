...
use FedExVendor\WPDesk\License\ServerAddressRepository;
 ...
 */
class LicenseActivation implements \FedExVendor\WPDesk\License\Page\Action
{
    /**
     * Plugin data.
     *
    ...
    public function execute(array $plugin)
    {
        ...
        $this->activate_license($activation_email, $api_key, new \FedExVendor\WPDesk\License\ServerAddressRepository($product_id));
    }
    /**
    ...
    {
        \add_settings_error('api_key_check_text', 'api_key_check_error', \__('Connection failed to the Subscription Key API server. Try again later.', 'flexible-shipping-fedex'), 'error');
    }
    /**
    ...
    {
        \add_settings_error('activate_text', 'activate_msg', \__('Plugin activated. ', 'flexible-shipping-fedex') . "{$activate_results['message']}.", 'updated');
    }
    /**
     * Activate license.
     *
     ...
     */
    public function activate_license($activation_email, $api_key, \FedExVendor\WPDesk\License\ServerAddressRepository $address_repository)
    {
