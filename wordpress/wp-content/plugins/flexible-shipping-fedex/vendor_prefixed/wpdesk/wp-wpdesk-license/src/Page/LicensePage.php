...
use FedExVendor\WPDesk\License\InstalledPlugins;
/**
 * Can render and manage license page.
 *
 * @package WPDesk\License\Page\License
 */
class LicensePage implements \FedExVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function __construct(\FedExVendor\WPDesk\License\InstalledPlugins $plugin_database)
    {
    ...
    {
        \add_submenu_page('wpdesk-helper', \__('Subscriptions', 'flexible-shipping-fedex'), \__('Subscriptions', 'flexible-shipping-fedex'), 'manage_options', self::PAGE_SLUG, [$this, 'handle_render_wpdesk_licenses_page']);
    }
    /**
    ...
    public function handle_render_wpdesk_licenses_page()
    {
        ...
        if (!\class_exists('FedExVendor\\WPDesk_Helper_List_Table')) {
    ...
    private function execute_plugin_action($plugin, $action)
    {
        ...
        if (isset($plugin_info)) {
            (new \FedExVendor\WPDesk\License\Page\LicensePageActions())->create_action($action)->execute($plugin_info);
        }
    }
    /**
