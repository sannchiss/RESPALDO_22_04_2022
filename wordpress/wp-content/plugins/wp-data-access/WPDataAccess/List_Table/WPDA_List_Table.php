...
class WPDA_List_Table extends Wordpress_Original\WP_List_Table
{
    ...
    public function __construct( $args = array() )
    {
        ...
        $this->pid = ( isset( $args['pid'] ) ? $args['pid'] : null );
        ...
        if ( isset( $_REQUEST["{$this->search_item_name}_old_value"] ) ) {
            $this->search_value_old = wp_unslash( $_REQUEST["{$this->search_item_name}_old_value"] );
            // input var okay.
        } else {
    ...
    public function column_default( $item, $column_name )
    {
        ...
        if ( $this->wpda_list_columns->get_table_columns()[0]['column_name'] === $column_name || $column_name === $this->first_display_column ) {
            ...
            if ( 0 === $count ) {
                ...
                // ??
                return $this->render_column_content( $item, $column_name ) . $this->row_actions( [
                    'wpda_hide_column' => '',
                ] );
            } else {
                // Check rights.
                ...
                if ( 'on' === $this->show_view_link ) {
                    ...
                    $view_form = "<form" . " id='view_form{$form_id}'" . " action='{$url}'" . " method='post'>" . $this->get_key_input_fields( $item ) . $this->add_parent_args_as_string( $item ) . "<input type='hidden' name='action' value='view' />" . $row_security_nonce_field . $this->page_number_item . "</form>";
                    ?>
                ...
                if ( 'on' === $this->allow_update || WPDA::is_wpda_table( $this->table_name ) ) {
                    // Build the row action.
                    // Use jQuery to add form to container.
                    $edit_form = "<form" . " id='edit_form{$form_id}'" . " action='{$url}'" . " method='post'>" . $this->get_key_input_fields( $item ) . $this->add_parent_args_as_string( $item ) . "<input type='hidden' name='action' value='edit' />" . $row_security_nonce_field . $this->page_number_item . "</form>";
                    ?>
                ...
                if ( 'on' === $this->allow_delete || WPDA::is_wpda_table( $this->table_name ) ) {
                    ...
                    // Use jQuery to add form to container.
                    $delete_form = "<form" . " id='delete_form{$form_id}'" . " action='{$url}'" . " method='post'>" . $this->get_key_input_fields( $item ) . $this->add_parent_args_as_string( $item ) . "<input type='hidden' name='action' value='delete' />" . "<input type='hidden' name='_wpnonce' value='{$wp_nonce}'>" . $row_security_nonce_field . $this->page_number_item . "</form>";
                    ?>
    ...
    public function show()
    {
        ...
        if ( $this->show_page_title ) {
            ...
            if ( self::LIST_BASE_TABLE !== $this->table_name && (current_user_can( 'manage_options' ) && \WP_Data_Access_Admin::PAGE_MAIN === $this->page) ) {
                ...
                ?>"
								...
								title="<?php 
                echo  ( \WP_Data_Access_Admin::PAGE_MAIN === $this->page ? __( 'Data Explorer', 'wp-data-access' ) : __( 'Favourites', 'wp-data-access' ) ) ;
                ?>"
						></a>
            ...
            switch ( $this->page ) {
                ...
                case WPDP::PAGE_MAIN:
                    ...
                    if ( 'WPDataProjects\\Project\\WPDP_Project_Project_List' === get_class( $this ) ) {
                        $help_url = 'https://wpdataaccess.com/docs/documentation/data-projects/data-projects-getting-started/';
                ...
                case WPDP::PAGE_TEMPLATES:
                    $help_url = 'https://wpdataaccess.com/docs/documentation/project-templates/gettings-started/';
                    $demo_url = '';
                    $tuts_url = '';
                    break;
                default:
            ...
            if ( '' !== $help_url ) {
                ...
                ?>" target="_blank"
							...
							<?php 
                if ( self::LIST_BASE_TABLE === $this->table_name && \WP_Data_Access_Admin::PAGE_MAIN === $this->page ) {
        ...
        ?>"/>
					<?php 
        wp_nonce_field( 'wpda-export-' . json_encode( $this->table_name ), '_wpnonce', false );
        ?>
					<?php 
					...
					>
						<div>
							...
							<button type="submit" class="page-title-action wpda_tooltip"
									title="<?php 
                echo  sprintf( __( 'Add new %s to %s', 'wp-data-access' ), $this->_args['singular'], $storage_type ) ;
                ?>"
							>
        ...
        switch ( $this->current_action() ) {
            case 'delete':
                ...
                foreach ( $this->wpda_list_columns->get_table_primary_key() as $key ) {
                    ...
                    $row_to_be_deleted[$i]['value'] = strip_tags( wp_unslash( $_REQUEST[$key] ) );
                    // input var okay.
                    $i++;
                    // Add key values to wp_nonce action.
                    $wp_nonce_action .= '-' . strip_tags( wp_unslash( $_REQUEST[$key] ) );
                    // input var okay.
                }
            ...
            case 'bulk-export-csv':
                ...
                $wp_nonce = ( isset( $_REQUEST['_wpnonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce'] ) ) : '' );
                // input var okay.
                if ( !wp_verify_nonce( $wp_nonce, 'wpda-export-' . json_encode( $this->table_name ) ) ) {
                    die( __( 'ERROR: Not authorized', 'wp-data-access' ) );
                }
                $bulk_rows = $_REQUEST['bulk-selected'];
                $no_rows = count( $bulk_rows );
                ...
                if ( 'diehard' === $this->page && null !== $this->pid ) {
                    ...
                    $querystring .= "?action=wpda_export&type=row&mysql_set=off&show_create=off&show_comments=off&pid={$this->pid}&_wpnonce={$wp_nonce}&format_type={$format_type}";
                } else {
                ...
                echo  '<i' . str_replace( '-', '', substr( $tag, 4 ) ) . " src='{$querystring}' style='display: none'></i" . str_replace( '-', '', substr( $tag, 4 ) ) . '>' ;
        }
    }
    ...
    protected function get_order_by()
    {
        ...
        if ( !empty($_REQUEST['orderby']) ) {
            $orderby_arg = sanitize_text_field( wp_unslash( $_REQUEST['orderby'] ) );
            // input var okay.
    ...
    protected function get_search_value()
    {
        if ( 'off' === WPDA::get_option( WPDA::OPTION_BE_REMEMBER_SEARCH ) ) {
            ...
            if ( isset( $_REQUEST[$this->search_item_name] ) ) {
                return wp_unslash( $_REQUEST[$this->search_item_name] );
                // input var okay.
            }
    ...
    public function print_column_headers( $with_id = true )
    {
        ...
        if ( isset( $_REQUEST['order'] ) && 'desc' === sanitize_text_field( wp_unslash( $_REQUEST['order'] ) ) ) {
            // input var okay.
            $current_order = 'desc';
        } else {
            $current_order = 'asc';
        ...
        foreach ( $columns as $column_key => $column_display_name ) {
            ...
            if ( isset( $sortable[$column_key] ) ) {
                ...
                echo  wp_kses( $column_display_name, [
                    'input' => [
                    'id'   => [],
                    'type' => [],
                ],
