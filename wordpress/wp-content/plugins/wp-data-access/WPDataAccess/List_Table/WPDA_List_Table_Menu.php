...
class WPDA_List_Table_Menu extends WPDA_List_Table
{
    ...
    public function single_row( $item )
    {
        ...
        if ( $this->bulk_actions_enabled ) {
            echo  '<tr style="height:0 !important; padding: 0 !important;"><td id="rownum_' . self::$list_number . '_x1" colspan="' . (13 - $column_not_showns) . '" style="padding:0 !important;"></tr>' ;
            ...
            echo  '<tr style="height:0 !important; padding: 0 !important;"><td id="rownum_' . self::$list_number . '_x1" colspan="' . (12 - $column_not_showns) . '" style="padding:0 !important;"></tr>' ;
            // Fake! Maintain odd/even colors.
    ...
    protected function process_bulk_action_drop_db()
    {
        ...
        if ( 'rdb:' === substr( $database, 0, 4 ) ) {
            ...
            if ( false === $wpdb->query( "drop database `{$database}`" ) ) {
    ...
    protected function process_bulk_action_create_db()
    {
        ...
        if ( isset( $_REQUEST['database_location'] ) && 'local' === $_REQUEST['database_location'] ) {
            ...
            if ( false === $wpdb->query( "create database `{$database}`" ) ) {
                // db call ok; no-cache ok.
                $msg = new WPDA_Message_Box( [
    ...
    protected function process_bulk_action_settings_table()
    {
        ...
        if ( isset( $_REQUEST['settings_table_name'] ) && isset( $_REQUEST['settings'] ) ) {
            ...
            if ( 'dashboard_menus' === $settings_object->request_type ) {
                ...
                if ( 'column_settings' === $settings_object->request_type ) {
    ...
    protected function post_drop_table( $drop_table_name )
    {
        ...
        // Table settings...
        $wpdb->query( $wpdb->prepare( 'delete from ' . WPDA_Table_Settings_Model::get_base_table_name() . ' where wpda_schema_name = %s ' . '   and wpda_table_name  = %s ', [ $this->schema_name, $drop_table_name ] ) );
        $wpdb->get_results();
        // WordPress media library columns...
        $wpdb->query( $wpdb->prepare( 'delete from ' . WPDA_Media_Model::get_base_table_name() . ' where media_schema_name = %s ' . '   and media_table_name  = %s ', [ $this->schema_name, $drop_table_name ] ) );
        $wpdb->get_results();
        // Data menus...
        $wpdb->query( $wpdb->prepare( 'delete from ' . WPDA_User_Menus_Model::get_base_table_name() . ' where menu_schema_name = %s ' . '   and menu_table_name  = %s ', [ $this->schema_name, $drop_table_name ] ) );
        $wpdb->get_results();
    ...
    protected function truncate_table( $table_name )
    {
        ...
        return $wpdadb->query( "truncate table `" . str_replace( '`', '', $table_name ) . "`" );
        // db call ok; no-cache ok.
    }
    ...
    protected function add_header_button( $add_param = '' )
    {
        ...
        $wpda_db_backups_activated = is_array( $wpda_db_options_activated ) && count( $wpda_db_options_activated ) > 0;
        
        if ( $wpda_db_backups_activated ) {
            $cron_jobs = _get_cron_array();
            $wpda_db_backups_activated = false;
            foreach ( $cron_jobs as $key => $cron ) {
                foreach ( $cron as $key => $value ) {
                    
                    if ( 'wpda_data_backup' === $key ) {
                        $wpda_db_backups_activated = true;
                        continue;
                    }
