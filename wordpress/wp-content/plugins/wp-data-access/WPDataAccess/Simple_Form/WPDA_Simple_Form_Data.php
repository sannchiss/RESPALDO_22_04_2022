			...
			foreach ( $this->wpda_list_columns->get_table_columns() as $column ) {
				if ( $column['column_name'] === $this->wpda_list_columns->get_auto_increment_column_name() ) {
					...
					if (
						...
						(
							null === $this->calling_form->get_new_value( $column['column_name'] ) ||
							'' === $this->calling_form->get_new_value( $column['column_name'] )
						)
					) {
						// Convert empty numeric column to null
				...
				if ( 1 === $result ) {
					...
					if ( 0 === $result && '' === $wpdadb->last_error ) {
						...
						if ( '' === $wpdadb->last_error ) {
							...
							if ( 1 === sizeof( $table_info ) && 'connect' === strtolower( $table_info[0]['engine'] ) ) {
