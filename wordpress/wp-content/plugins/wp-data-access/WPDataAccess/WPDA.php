...
namespace WPDataAccess {
	...
	class WPDA {
		 ...
		 */
		const OPTION_WPDA_VERSION = [ 'wpda_version', '4.3.0' ];
		/**
		...
		const OPTION_PLUGIN_NAVIGATION = [ 'wpda_plugin_navigation', 'menu' ];
		...
		const OPTION_DP_STYLE_COMPACT     = [ 'wpda_dp_style_compact', 'off' ];
		...
		public static function validate_names( $schema_name, $table_name, $column_names = null ) {
			...
			if ( 'rdb:' === substr( $schema_name, 0, 4 ) ) {
				...
				if ( ! self::validate_name( $schema_name ) ) {
					return self::validate_name_failed();
				}
			}
