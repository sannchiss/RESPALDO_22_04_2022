...
class WPDP_Project_Table_Form
{
    ...
    public function get_unique_setname()
    {
        ...
        global  $wpdb ;
        $query = "select 'x' from " . WPDP_Project_Design_Table_Model::get_base_table_name() . ' where wpda_table_name = %s';
        $wpdb->get_results( $wpdb->prepare( $query, [ $this->wpda_table_name ] ) );
        
        if ( $wpdb->num_rows > 0 ) {
            $query = "select 'x' from " . WPDP_Project_Design_Table_Model::get_base_table_name() . ' where wpda_schema_name = %s and wpda_table_name = %s and wpda_table_setname = %s';
            $i = $wpdb->num_rows + 1;
    ...
    {
        $title = __( 'Project Templates', 'wp-data-access' );
        ?>
			<a
				href="?page=<?php 
    ...
    protected function show_table_info()
    {
        ...
        $defaul_where = ( isset( $this->table_structure->tableinfo->default_where ) ? $this->table_structure->tableinfo->default_where : '' );
        $defaul_orderby = ( isset( $this->table_structure->tableinfo->default_orderby ) ? $this->table_structure->tableinfo->default_orderby : '' );
        $hyperlinks_parent = ( isset( $this->table_structure->tableinfo->hyperlinks_parent ) ? $this->table_structure->tableinfo->hyperlinks_parent : [] );
        $hyperlinks_child = ( isset( $this->table_structure->tableinfo->hyperlinks_child ) ? $this->table_structure->tableinfo->hyperlinks_child : [] );
        $html = '';
        $html_child = '';
			...
			<script type='text/javascript'>
				...
				function wpdp_get_tables(schema_name, index, target_id, selected_value, target_column_name = false, source_column_name= false) {
					...
					var data = {
						...
						wpda_wpnonce: '<?php 
        echo  wp_create_nonce( "wpda-getdata-access-" . WPDA::get_current_user_login() ) ;
        ?>'
					};
					jQuery.post(
						url,
                    ...
                    if ( $column_name === $relationship->source_column_name[0] && ('lookup' === $relationship->relation_type || 'autocomplete' === $relationship->relation_type) ) {
                        ...
                        foreach ( $lookup_column_list as $lookup_column ) {
                            ...
                            ?>
														>
															<?php 
                            echo  $lookup_column['column_name'] ;
                            ?>
														</option>
														<?php 
                        }
