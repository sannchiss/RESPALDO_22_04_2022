...
function posterity__no_meta_posts_page_notice() {
    echo '<div class="notice notice-warning inline"><p>' .
         sprintf(
             wp_kses(
                 /* translators: %1$s: link to customizer, %2$s section name */
                 __( 'Theme options for this page can be found in <a href="%1$s">Appearance -&gt; Customize -&gt; %2$s</a>.', 'posterity' ),
                 array(
                     'a'      => array(
                         'href' => array()
                     )
             ...
             esc_html__( 'Blog settings', 'posterity' )
         ) . '</p></div>';
}

function posterity__no_meta_shop_page_notice() {
    echo '<div class="notice notice-warning inline"><p>' .
         sprintf(
             wp_kses(
                /* translators: %1$s: link to customizer, %2$s section name */
             ...
             esc_html__( 'Shop(WooCommerce) settings', 'posterity' )
         ) . '</p></div>';
}
 ...
 */
function posterity_admin_meta_boxes(){
    global $posterity_a13;

    if(current_user_can( 'edit_posts' )){
        add_meta_box(
            'sktwb_theme_options',
             __( 'Blog post details', 'posterity' ),
            'posterity_meta_main_opts',
            'post',
            'normal',
            'default',
            array('func' => 'posterity_meta_boxes_post')//callback
        );
    }
    ...
    if(current_user_can( 'edit_pages' )){
        ...
        if($current_page_id == get_option( 'page_for_posts' )){
            add_meta_box(
                ...
                __( 'Page details', 'posterity' ),
                'posterity__no_meta_posts_page_notice',
                'page',
                'normal',
                'default'
            );
        ...
        //shop page
        elseif(posterity_is_woocommerce_activated() &&( $current_page_id == wc_get_page_id( 'shop' ) )){
            add_meta_box(
                'sktwb_theme_options_notice',
                __( 'Page details', 'posterity' ),
                'posterity__no_meta_shop_page_notice',
                'page',
                'normal',
                'default'
            );
            ...
            add_meta_box(
                ...
                'normal',
                'default',
                array('func' => 'posterity_meta_boxes_page')//callback
            );
        }
    }

}
add_action( 'add_meta_boxes', 'posterity_admin_meta_boxes');
 ...
 */
function posterity_meta_main_opts( $post, $meta_box ){
            ...
            */
            ...
            if ( $meta['type'] === 'fieldset' ) {
                ...
                if( isset( $meta['companion_required'] ) && $meta['companion_required'] === true ){
                    posterity_is_companion_plugin_ready();
                }
            ...
            elseif( posterity_print_form_controls($meta, $params, true ) ){
                continue;
            }
            ...
            elseif ( $meta['type'] === 'multi-upload' ) { ?>
                ...
                <div class="a13-mu-container">
                    ...
                    echo esc_attr( __( 'Select/Upload images and videos', 'posterity' ) );
                    ?>"<?php echo ( isset( $meta['media_type'] ) && strlen( $meta['media_type'] ) ) ?
                        ' data-media-type="'.esc_attr($meta['media_type']) . '"' : ''; ?> />
                    <span class="button button-large add-link-media"><?php esc_html_e( 'Add Video from Youtube/Vimeo', 'posterity' ); ?></span>
                    <label class="button button-large"><input type="checkbox" id="mu-prepend" value="1" /><?php esc_html_e( 'Add items at beginning of the list', 'posterity' ); ?>
                    </label>
                    <input id="a13-multi-remove" class="button button-large" type="button" value="<?php echo esc_attr( __( 'Remove selected', 'posterity' ) ); ?>" disabled="disabled" />
                    <?php

                    do_action('posterity_a13_multi_upload_tools');

                    posterity_input_help_tip( __( 'To mark more items in Media Library and in below list, you can use <code>Ctrl</code>(<code>Cmd</code>) or <code>Shift</code> key while selecting them with mouse.', 'posterity' ));
                    ?>
                ...
                posterity_admin_gallery_item_html( 'attachment-preview image', 'thumbnail', get_theme_file_uri( 'images/holders/video_150x100.png') );
                echo '</div>';
                ?>
                <ul id="mu-media" class="media-frame-content" data-columns="5">
                    <?php posterity_prepare_admin_gallery_html( posterity_prepare_gallery_attachments( $value ) ); ?>
                </ul><?php
            }
        } //end foreach
    } //end foreach
 ...
 * @param int $post_id
 */
function posterity_save_post($post_id){
    static $done = 0;
    $done++;
    if( $done > 1 ){
        ...
        foreach ( $meta_boxes as $meta_tab ) {
            foreach( $meta_tab as $meta ) {
                ...
                if( isset( $meta['id'] ) && isset( $_POST[ $input_prefix . $meta['id'] ] ) ){
                    ...
                    update_post_meta( $post_id, '_' . $meta['id'], apply_filters( 'posterity_save_post_meta', $val, $meta['id'] ) );
                }
            }
        }
    }
}
add_action( 'save_post', 'posterity_save_post' );